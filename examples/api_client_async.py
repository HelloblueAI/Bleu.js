    ...
    async with AsyncBleuAPIClient() as client:
        ...
        try:
            response = await client.chat(
                [
                    {
                        "role": "user",
                        "content": "Explain async programming in one sentence.",
                    }
                ]
            )
        ...
        try:
            response = await client.generate(
                prompt="The future of AI is", max_tokens=100
            )
        ...
        try:
            ...
            tasks = [client.generate(prompt, max_tokens=50) for prompt in prompts]
