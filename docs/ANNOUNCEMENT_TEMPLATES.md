# 📣 Bleu.js v1.2.1 Announcement Templates

## 🐦 Twitter/X Post

```
🚀 Bleu.js v1.2.1 is live! Python 3.12 ready & cleaner installs than ever.

✅ Certified on Python 3.12.2
✅ Pydantic v2 migration complete (no deprecation noise)
✅ API client ships with every wheel/sdist
...
```
🚀 Announcing Bleu.js v1.2.1 – Seamless Python 3.12 Compatibility

We’ve just published Bleu.js 1.2.1 to PyPI, ensuring every customer can upgrade without friction while keeping the powerful features from 1.2.0.

🔧 Compatibility & Stability
• Fully validated on Python 3.12.2
• Pydantic v2 migration complete (zero deprecation warnings)
• Optional GPU/Ray dependencies now guarded for graceful fallbacks

📦 Packaging Improvements
• API client included automatically in every wheel/sdist
• Lean 125 KB wheel for ultra-fast installs
• Updated docs, quick starts, and changelog to guide users

🤝 User Impact
• No manual steps required—`pip install bleu-js==1.2.1`
• Cloud API client ready out of the box (`from bleujs.api_client import BleuAPIClient`)
• Supports teams standardizing on the latest Python toolchain

Ready to upgrade? ➡️ `pip install --upgrade bleu-js==1.2.1`
Release notes: docs/RELEASE_ANNOUNCEMENT_v1.2.1.md
...
```
Subject: Bleu.js 1.2.1 – Python 3.12 Ready + API Client Included

Hello team,

We’ve shipped Bleu.js v1.2.1 to PyPI. This patch release focuses on compatibility and packaging polish:
• Certified Python 3.12 support (pyenv toolchain, fresh virtualenv)
• Completed Pydantic v2 migration—no more deprecated validator warnings
• API client bundled automatically; optional deps remain optional
• Docs and quick starts refreshed so users upgrade smoothly

Recommended command:
    pip install --upgrade 'bleu-js[api]'==1.2.1

Questions? Check docs/RELEASE_ANNOUNCEMENT_v1.2.1.md or ping the engineering team.
...
## 🛠️ Internal Slack / Teams Update

```
:tada: Release update: Bleu.js v1.2.1 is live on PyPI! Highlights:
• Python 3.12 certified
• Pydantic v2 clean-up (no warnings)
• API client now guaranteed in every install
• Optional GPU/Ray deps wrapped for graceful fallback

Docs to share:
• Release brief: docs/RELEASE_ANNOUNCEMENT_v1.2.1.md
• Install guide: docs/INSTALLATION_FOR_USERS.md

Encourage customers/advocates to upgrade with:
pip install --upgrade bleu-js==1.2.1
```

---

## 📄 Press / Blog Opening Paragraph

```
Bleu.js 1.2.1 builds on our landmark 1.2.0 release with a sharp focus on compatibility. The new version certifies Python 3.12 support, completes our Pydantic v2 migration, and guarantees the Bleu.js cloud API client ships in every distribution. Customers can now upgrade without manual steps, enjoying faster installs and a cleaner developer experience.
```

---

# 📣 Bleu.js v1.2.0 Announcement Templates (Archive)
