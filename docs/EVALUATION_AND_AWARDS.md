...
- **Standards:** We follow a strict [security policy](../SECURITY.md), a single source of truth for [dependencies and Dependabot](DEPENDABOT_AND_DEPENDENCIES.md), and the same growing approach as Node.js, Fuse.js, and other .js projects (multi-repo, contract-first, community-first).
...
| **Changing the API (runbook)** | [Changing the API](CHANGING_THE_API.md) — checklist so both repos stay in sync |
...
- **Contract-first:** The API contract lives in one place ([openapi.yaml](api/openapi.yaml) + doc table); both the SDK and the backend point to it. The spec is validated in CI on change. Changes are visible and traceable.
- **Accelerated flow:** A [Changing the API runbook](CHANGING_THE_API.md) gives a single checklist (contract → backend → client) so we ship faster than teams that don't document the flow. Backend CONTRIBUTING includes a sync checklist.
