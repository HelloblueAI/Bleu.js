...
document.getElementById('fetchDataButton').addEventListener('click', () => {
  ...
  fetch('http://localhost:4003/api/data') // Ensure correct backend port
    .then((response) => {
      if (!response.ok) {
...
document
  ...
  .addEventListener('click', () => {
    ...
    fetch('http://localhost:3002/dependencies') // Ensure correct dependency-management port
      .then((response) => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
...
document
  ...
  .addEventListener('click', () => {
    ...
    fetch('http://localhost:3002/dependencies/conflicts') // Ensure correct dependency-management port
      .then((response) => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
