...
async def predict(input_data: PredictionInput):
    ...
    try:
        ...
        if features_array.shape[0] < expected_features:
            return {"error": f"❌ Too few features: expected {expected_features}, got {features_array.shape[0]}"}

        elif features_array.shape[0] > expected_features:
            return {"error": f"❌ Too many features: expected {expected_features}, got {features_array.shape[0]}"}
