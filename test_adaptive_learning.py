...
from adaptive_learning import QuantumAwareLRConfig, QuantumAwareScheduler
...
def config():
    ...
    return QuantumAwareLRConfig(
        ...
        quantum_sensitivity=0.5,  # Increased sensitivity for testing
        measurement_frequency=1,  # Measure every step
...
def test_quantum_impact_adjustment(optimizer, config):
    ...
    class MockQuantumProcessor:
        ...
        def get_state(self):
            ...
            if self.call_count == 1:
                ...
                # Return a state with a large distance from the initial state
                return np.array([0.9, 1.0, 1.1])
    ...
    # First step to record initial state
    scheduler.step()

    # Second step to trigger quantum adjustment
    scheduler.step()
