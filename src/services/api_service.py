...
from ..models import User, Subscription, APIUsage, APICall
...
# Prometheus metrics
api_calls_total = Counter('api_calls_total', 'Total number of API calls', ['endpoint', 'status'])
api_response_time = Histogram('api_response_time_seconds', 'API response time in seconds', ['endpoint'])
api_errors_total = Counter('api_errors_total', 'Total number of API errors', ['endpoint', 'error_type'])
...
class APIService:
    ...
    async def check_usage_limit(self, user: User) -> bool:
        ...
        monthly_usage = self.db.query(APIUsage).filter(
    ...
    async def track_api_call(self, user: User, endpoint: str, response_time: float, status: int):
        ...
        api_usage_gauge.labels(user_id=str(user.id), plan=user.subscription.plan_type).set(monthly_usage.calls_count)
    ...
    async def get_usage_analytics(self, user: User) -> Dict:
        ...
        # Get daily usage
        daily_usage = self.db.query(APICall).filter(
            APICall.user_id == user.id,
    ...
    async def get_advanced_analytics(self, user: User) -> Dict:
        ...
        if user.subscription.plan_type != "ENTERPRISE":
            raise HTTPException(status_code=403, detail="Advanced analytics only available for Enterprise plan")
    ...
    async def get_user_dashboard_data(self, user: User) -> Dict:
        ...
        dashboard_data = {
            ...
            "usage": {
                "calls_remaining": user.subscription.api_calls_limit - usage_analytics["total_calls"],
                "current_usage": usage_analytics["total_calls"],
                "response_time_distribution": usage_analytics["response_time_distribution"],
    ...
    async def monitor_api_health(self) -> Dict:
        ...
        system_metrics = {
            ...
            "error_rate": api_errors_total._value.get() / api_calls_total._value.get() if api_calls_total._value.get() > 0 else 0
        }
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(settings.QUANTUM_SERVICE_URL + "/health") as response:
                    quantum_health = "healthy" if response.status == 200 else "unhealthy"
            except Exception as e:
    ...
    async def optimize_api_performance(self) -> Dict:
        ...
        recent_calls = self.db.query(APICall).filter(
            APICall.timestamp >= datetime.now(UTC) - timedelta(hours=24)
        ).all()
