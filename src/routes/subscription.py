...
from src.models.user import UserResponse, User
...
async def get_current_subscription(
    ...
    services = init_services(db)
    return await services['subscription_service'].get_user_subscription(current_user.id, db)
...
async def cancel_subscription(
    ...
    services = init_services(db)
    subscription = await services['subscription_service'].get_user_subscription(current_user.id, db)
    if not subscription:
        raise HTTPException(
