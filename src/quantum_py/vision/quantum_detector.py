...
class QuantumDetector:
            ...
            if detections:
                ...
                if detection_features:
                    ...
                    if self.temporal_analyzer is not None and temporal_context is not None:
                        temporal_context_data = await self._analyze_temporal_context(temporal_context)
                    ...
                    for i, detection in enumerate(detections):
                        ...
                        detection.temporal_context = temporal_context_data if self.use_temporal_context else None
    ...
    async def _analyze_scene_context(
        self,
        image: torch.Tensor
    ) -> Dict:
        """Analyze scene context"""
    ...
    async def _apply_self_learning(
        self,
    ...
    def _update_metrics(self, detections: List[DetectionResult]):
        ...
        if self.quantum_processor is not None:
            ...
            self.performance_metrics['quantum_speedup'] = quantum_metrics.get('quantum_speedup', 1.0)
        
        # Update scene accuracy if scene analyzer is available
        if self.scene_analyzer is not None:
            scene_confidences = [d.scene_context.get('scene_confidences', [0.0])[0] for d in detections]
        ...
        if self.temporal_analyzer is not None:
            temporal_confidences = [d.temporal_context.get('temporal_confidences', [0.0])[0] for d in detections if d.temporal_context]
