...
class QuantumAlgorithms:
    ...
    def grover_search(self,
                     ...
                     num_iterations: Optional[int] = None) -> List[int]:
    ...
    def quantum_counting(self,
                        oracle: np.ndarray,
                        num_qubits: int,
                        precision_qubits: int) -> Tuple[int, float]:
        ...
        for i in range(precision_qubits):
            ...
            for _ in range(power):
                circuit.add_gate('CUSTOM', list(range(num_qubits)), 
                               matrix=oracle, control_qubits=[i])
                circuit.add_gate('CUSTOM', list(range(num_qubits)), 
                               matrix=G, control_qubits=[i])
                
        # Apply inverse QFT to counting qubits
        qft_circuit = self.quantum_fourier_transform_circuit(precision_qubits, 
                                                           inverse=True)
