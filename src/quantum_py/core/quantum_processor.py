...
class QuantumProcessor:
    ...
    def apply_circuit(self, circuit: QuantumCircuit) -> np.ndarray:
        ...
        if len(circuit.gates) > self.config.max_depth:
            raise ValueError(f"Circuit depth {len(circuit.gates)} exceeds maximum {self.config.max_depth}")
        ...
        for gate in circuit.gates:
            ...
            if self.config.error_rate > 0:
                noise_func = self.noise_models.get(self.config.noise_model, self._apply_depolarizing_noise)
    ...
    def __str__(self) -> str:
        ...
        return f"QuantumProcessor(num_qubits={self.config.num_qubits}, " \
