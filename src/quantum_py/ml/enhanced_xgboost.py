...
class EnhancedXGBoost:
            ...
            if eval_set:
                ...
                for val_features, val_labels in eval_set:
                    if val_features.shape[1] != self.n_features:
                        raise ValueError(f"Validation set feature shape mismatch, expected: {self.n_features}, got {val_features.shape[1]}")
    ...
    async def predict(
        ...
        try:
            ...
            if features.shape[1] != self.n_features:
                raise ValueError(f"Feature shape mismatch, expected: {self.n_features}, got {features.shape[1]}")
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        if self.feature_importance is None:
            raise ValueError("Model has not been trained yet or does not support feature importance")
