...
class QuantumProcessor:
    ...
    def _update_metrics(self, features: np.ndarray, circuit_info: Dict[str, Any]):
        ...
        self.metrics.update({
            ...
            'error_rate': 1 - (self.metrics['successful_executions'] + 1) / (self.metrics['total_executions'] + 1),
            'quantum_speedup': 2.0 * np.exp(-circuit_info['metrics'].get('circuit_depth', 0) / 100),
            ...
            'circuit_optimization_score': circuit_info['metrics'].get('optimization_score', 0.0)
        })
