...
class AdvancedQuantumDetector:
    ...
    def _update_metrics(self, results: List[DetectionResult], 
                       start_time: float, memory_util: float):
        ...
        self.metrics['detection_metrics'].update({
            'total_detections': self.metrics['detection_metrics']['total_detections'] + n_detections,
            'successful_detections': self.metrics['detection_metrics']['successful_detections'] + n_confident,
            'failed_detections': self.metrics['detection_metrics']['failed_detections'] + (n_detections - n_confident),
        ...
        self.metrics['quantum_metrics'].update({
            ...
            'entanglement_quality': np.mean([r.uncertainty for r in results]) if results else 0.0,
            'quantum_advantage': quantum_advantage,
            'qubit_stability': 1.0 - np.std([r.confidence for r in results]) if results else 0.0
        })
        ...
        self.metrics['learning_metrics']['adaptation_score'] = np.mean(
            self.metrics['performance_metrics']['energy_efficiency'][-100:]
        ) if self.metrics['performance_metrics']['energy_efficiency'] else 0.0
