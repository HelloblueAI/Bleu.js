...
def quantum_mutual_information(state: np.ndarray, subsys_a: List[int], 
                             subsys_b: List[int], num_qubits: int) -> float:
    ...
    rho_ab = partial_trace(state, list(set(range(num_qubits)) - set(subsys_a + subsys_b)), num_qubits)
...
def create_phase_estimation_circuit(U: np.ndarray, precision_qubits: int) -> Tuple[np.ndarray, List[int]]:
