            ...
            for step_batch in execution_order:
                ...
                for step_name, result in zip(step_batch, results):
                    if isinstance(result, Exception):
                        self.logger.error(
                            f"Step '{step_name}' failed: {str(result)}"
                        )
                        error_count += 1
    ...
    def _update_metrics(
        ...
        if self.monitoring_enabled:
            metrics = PipelineMetrics(
                ...
                success_rate=(len(self.steps) - error_count) / len(self.steps) if self.steps else 0.0,
