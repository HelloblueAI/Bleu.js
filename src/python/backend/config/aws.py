...
class AWSConfig(BaseModel):
    ...
    authorizer_type: str = Field(default="JWT", description="API Gateway authorizer type")
    ...
    cache_key_parameters: list[str] = Field(
        default=["user_id"],
        description="Parameters to use for cache key"
    )
    integration_timeout: int = Field(
        default=29000,
        description="Integration timeout in milliseconds"
    )
    request_validator: Optional[str] = Field(
        None,
        description="Request validator name"
    )
    request_models: dict[str, str] = Field(
        default_factory=dict,
        description="Request models for validation"
    )
    response_models: dict[str, str] = Field(
        default_factory=dict,
        description="Response models for validation"
    )
    binary_media_types: list[str] = Field(
        default=["*/*"],
        description="Binary media types supported"
    )
    ...
    policy: Optional[str] = Field(
        None,
        description="API Gateway resource policy"
    )
