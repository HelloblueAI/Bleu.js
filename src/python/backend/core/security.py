...
class SecurityManager:
    ...
    def _setup_ip_filtering(self):
        ...
        self.allowed_networks = [
            ip_network(ip) for ip in self.config.allowed_ips or []
        ]
        self.blocked_networks = [
            ip_network(ip) for ip in self.config.blocked_ips or []
        ]
    ...
    def check_rate_limit(self, client_ip: str) -> bool:
        ...
        if len(requests) >= self.config.rate_limit.requests_per_minute:
            if len(requests) >= self.config.rate_limit.burst_size:
                self.blocked_ips[client_ip] = now + timedelta(minutes=self.config.rate_limit.block_duration_minutes)
                return False
            return False
...
security_manager = SecurityManager(SecurityConfig(
    secret_key="your-secret-key-here",  # Replace with secure secret key
