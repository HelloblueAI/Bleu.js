...
class EnsembleManager:
    ...
    async def _tune_hyperparameters(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        for i, model in enumerate(self.models):
            if isinstance(model, (RandomForestClassifier, GradientBoostingClassifier)):
                ...
                study.optimize(
                    lambda trial: self._objective(trial, model, X, y),
                    n_trials=50
                )
    ...
    async def _calculate_kappa(self, X: np.ndarray) -> float:
        ...
        predictions = np.array(predictions)
        agreement = np.mean([np.mean(predictions[i] == predictions[j])
                           for i in range(len(self.models))
                           for j in range(i + 1, len(self.models))])
        
        random_agreement = np.mean([np.mean(predictions[i] == predictions[j])
                                  for i in range(len(self.models))
                                  for j in range(i + 1, len(self.models))
                                  if i != j])
    ...
    async def predict(
        ...
        if self.config.voting_method == 'soft':
            ...
            if self.weights is not None:
                ensemble_pred = np.average(predictions > 0.5, axis=0, weights=self.weights)
            else:
