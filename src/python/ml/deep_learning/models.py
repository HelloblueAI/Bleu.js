...
class EnhancedBaseModel(nn.Module):
    ...
    def export_onnx(self, sample_input: torch.Tensor, path: str):
        ...
        torch.onnx.export(self, sample_input, path,
                         input_names=['input'],
                         output_names=['output'],
                         dynamic_axes={'input': {0: 'batch_size'},
                                     'output': {0: 'batch_size'}})
        
    def apply_knowledge_distillation(self, teacher_model: 'EnhancedBaseModel',
        ...
        def distillation_loss(student_logits, teacher_logits, labels, alpha=0.1):
            ...
            distillation = nn.KLDivLoss(reduction='batchmean')(student_log_probs, soft_targets)
