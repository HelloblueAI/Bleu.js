...
def load_and_preprocess_data(data_dir: str, 
                           batch_size: int,
                           img_size: Tuple[int, int]) -> Tuple[tf.data.Dataset, tf.data.Dataset]:
    ...
    train_ds = train_ds.map(
        lambda x, y: (train_augmentation(x), y),
        num_parallel_calls=tf.data.AUTOTUNE
    )
    
    val_ds = val_ds.map(
        lambda x, y: (val_preprocessing(x), y),
        num_parallel_calls=tf.data.AUTOTUNE
    )
...
def main():
    ...
    parser.add_argument("--quantum_layers", type=int, default=3, help="Number of quantum layers")
    parser.add_argument("--quantum_qubits", type=int, default=4, help="Number of quantum qubits")
