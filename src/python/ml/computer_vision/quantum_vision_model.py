...
class QuantumVisionModel:
    ...
    def _build_model(self) -> None:
        ...
        try:
            ...
            if self.config.use_attention:
                features = self.attention.compute_attention(features, features, features)
            ...
            self.model.compile(
                ...
                loss=self.loss.quantum_cross_entropy if self.config.use_quantum_loss else 'categorical_crossentropy',
                metrics=['accuracy']
            )
    ...
    def _multi_scale_processing(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        for scale in self.config.scale_factors:
            ...
            x = tf.image.resize(x, (self.config.input_shape[0], self.config.input_shape[1]))
            features.append(x)
