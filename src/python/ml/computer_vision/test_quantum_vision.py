...
def load_test_data(data_dir: str, 
                   batch_size: int,
                   img_size: Tuple[int, int]) -> tf.data.Dataset:
...
def visualize_predictions(model: QuantumVisionModel,
    ...
    for images, labels in test_ds.take(num_samples):
        ...
        for i, (image, label, pred) in enumerate(zip(images, labels, predictions)):
            ...
            # Add text to image
            cv2.putText(image, f"True: {true_class}", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv2.putText(image, f"Pred: {pred_class}", (10, 70),
