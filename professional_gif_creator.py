...
class ProfessionalGIFCreator:
    ...
    def check_dependencies(self):
        ...
        if (
            not subprocess.run(["which", "convert"], capture_output=True).returncode
            == 0
        ):
            self.print_status("Installing ImageMagick...", "WARNING")
            subprocess.run(
                ["sudo", "apt-get", "install", "-y", "imagemagick"], check=True
            )
    ...
    def create_asciinema_gif(self):
        ...
        try:
            ...
            # Create the GIF
            subprocess.run(
                ["asciinema-gif", str(self.cast_file), str(self.output_gif)], check=True
            )
            ...
            self.print_status(
                "asciinema-gif not available, trying alternative method", "WARNING"
            )
            return False
    ...
    def create_ffmpeg_gif(self):
        ...
        try:
            ...
            subprocess.run(
                ["asciinema", "play", str(self.cast_file), "--speed", "1"], check=True
            )
    ...
    def create_professional_placeholder(self):
        ...
        for i in range(60):  # 6 seconds at 10fps
            ...
            draw.text(
                (subtitle_x, 160), subtitle, fill=secondary_color, font=font_medium
            )
    ...
    def run(self):
        ...
        try:
            ...
            if self.create_final_gif():
                ...
                print(
                    f"📤 Upload to: https://github.com/HelloblueAI/Bleu.js/assets/81389644/"
                )
